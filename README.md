...
社員は仕事の前に Google検索で裏を取ります。参照したページのURLは、成果物の末尾に自動で付きます。

止めたい場合は、次のどちらかです。

| 止めたい範囲 | やること |
|---|---|
| 全社員 | `.github/workflows/company.yml` の `ENABLE_SEARCH` を `"false"` |
| 特定の社員だけ | `company/roster.json` でその社員を `"search": false` |
...
校閲のように調べる必要のない役割は止めておくと、無料枠が長持ちします。

検索付きの呼び出しは通常の呼び出しとは別枠で数えられます。無料枠の条件は各サービスの公式ページで確認してください。
